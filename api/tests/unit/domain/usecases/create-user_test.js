...
describe('Unit | UseCase | create-user', () => {
  ...
  const savedAuthenticationMethod = domainBuilder.buildAuthenticationMethod.buildWithHashedPassword({
    userId,
    hashedPassword,
  });
  ...
  beforeEach(() => {
    ...
    userService.createUserWithPassword.resolves({
      savedUser,
  ...
  context('when user is valid', () => {
    ...
    it('should return saved user and user\'s authenticationMethod', async () => {
      // when
      const {
        savedUser: createdUser,
        savedAuthenticationMethod: createdAuthenticationMethod,
      } = await createUser({
        user,
        password,
        campaignCode,
      ...
      expect(createdAuthenticationMethod).to.deep.equal(savedAuthenticationMethod);
    });
  });
});
