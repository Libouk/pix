...
const learningContentWithAllChallenges = _learningContent({ withFrenchSpokenChallenges: false });
const learningContentWithFrenchSpokenChallenges = _learningContent({ withFrenchSpokenChallenges: true });

function _learningContent({ withFrenchSpokenChallenges }) {
  return [{
    id: 'recArea0',
    competences: [{
      id: competenceId,
      ...
      tubes: [{
        ...
        skills: [{
          ...
          id: 'skillWeb2',
          nom: '@skillWeb2',
          challenges: withFrenchSpokenChallenges ? [
  ...
  describe('GET /api/assessments/:assessment_id/next', () => {
    ...
    context('when assessment is a competence evaluation', () => {
      ...
      context('when there is one challenge in the accepted language (fr-fr)', () => {
        beforeEach(async () => {
          const learningContentObjects = learningContentBuilder.buildLearningContent(learningContentWithAllChallenges);
      ...
      context('when there is no challenge in the accepted language (fr-fr)', () => {
        beforeEach(async () => {
          const learningContentObjects = learningContentBuilder.buildLearningContent(learningContentWithFrenchSpokenChallenges);
          mockLearningContent(learningContentObjects);
        ...
        it('should return the challenge in the fallback language (fr)', () => {
          ...
          // then
          return promise.then((response) => {
            expect(response.result.data.id).to.equal(frenchSpokenChallengeId);
          });
        });
      });
    });
